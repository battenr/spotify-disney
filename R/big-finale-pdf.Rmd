...
We're going to need to see what's going on in these nested lists. Below is an example, again using The Lion King (1994): 
...
This will show list after list. Take your time to scroll through and see what kind of data is available!

## Step 7: Popularity

Now, we want to see how popular each of these songs are. There is a variable labeled popularity, which ranges from 0 to 100, with 100 being the highest / most popular. Popularity is not static rather it is dynamic, so it is liable to change over time. Anyways, at the time of this writing: May 9, 2021 the results are (drummmmrollll pleaseee):
...
big_finale <- big_finale %>% 
  dplyr::arrange(
    -popularity # organizing table by popularity
  ) 
